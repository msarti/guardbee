...
def findUser(user_id: String): Option[U]
  ...
  def getAutorization[A](access_token: AccessToken, scope: String)(implicit request: Request[A]): Either[PlainResult, GrantedAuthorization] = {
    val authorization = GrantedAuthorizationService.findByClientIdAndUser(access_token.client_id, access_token.user)
    authorization match {
