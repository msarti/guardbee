...
abstract class TemplateProvider(app: Application) extends BasePlugin {
  ...
  def appApprovalPage(title: String, client_id: Option[ClientID], scope: Seq[Option[Scope]], auth_code: Option[guardbee.services.AuthCode])(implicit request: Request[AnyContent]): Html
...
object TemplateProvider extends ServiceCompanion[TemplateProvider] {
  ...
  def loginPage()(implicit flash: Flash): Html = {
    this.getDelegate.map(_.loginPage()(flash)) getOrElse {
      this.notInitialized
      null
    }
  }
  ...
  def errorPage(title:String, messages:Seq[String]): Html = {
   ...
   def appApprovalPage(title: String, client_id: Option[ClientID], scope: Seq[Option[guardbee.services.Scope]], auth_code: Option[guardbee.services.AuthCode])(implicit request: Request[AnyContent]): Html = {
